...
class AiRequestLogStoreTest {
    ...
    fun `api key in query is redacted`() {
        assertThat(AiRequestLogStore.redactUrl("https://generativelanguage.googleapis.com/v1beta/models?key=SECRET"))
            .isEqualTo("https://generativelanguage.googleapis.com/v1beta/models?key=***")
    }

    @Test
    fun `only credential params are redacted`() {
        assertThat(AiRequestLogStore.redactUrl("https://host/v1?model=mimo&access_token=SECRET&stream=false"))
            .isEqualTo("https://host/v1?model=mimo&access_token=***&stream=false")
    }

    @Test
    fun `param without value is kept`() {
