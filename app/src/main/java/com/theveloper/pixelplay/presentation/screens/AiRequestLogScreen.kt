...
import com.theveloper.pixelplay.presentation.viewmodel.AiRequestLogViewModel
...
fun AiRequestLogScreen(
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        TopAppBar(
            ...
            navigationIcon = {
                IconButton(onClick = {
                    if (selected != null) selectedId = null else navController.popBackStack()
                }) {
            ...
            actions = {
                if (selected == null && logs.isNotEmpty()) {
                    IconButton(onClick = { askClearAll = true }) {
                        Icon(Icons.Rounded.DeleteSweep, contentDescription = stringResource(R.string.ai_request_log_clear_all))
                    }
                }
            }
        )
        ...
        if (logs.isEmpty()) {
            ...
            AiRequestLogDetail(log = selected, viewModel = viewModel)
        } else {
            LazyColumn(
    ...
    if (askClearAll) {
        AlertDialog(
            ...
            text = { Text(stringResource(R.string.ai_request_log_clear_all_message, logs.size)) },
            confirmButton = {
                TextButton(onClick = {
                    viewModel.clearAll()
                    askClearAll = false
            ...
            dismissButton = {
                TextButton(onClick = { askClearAll = false }) { Text(stringResource(R.string.common_cancel)) }
            }
        )
    }
}
...
private fun AiRequestLogRow(log: AiRequestLog, onClick: () -> Unit) {
    ...
    Surface(
        ...
        Column(modifier = Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            Text(
                text = if (log.model.isBlank()) log.provider else "${log.provider} · ${log.model}",
        ...
        Text(
            text = stringResource(
                when (status) {
                    ...
                    AiRequestLog.STATUS_FROM_CACHE -> R.string.ai_request_log_status_cached
...
@Composable
private fun AiRequestLogDetail(log: AiRequestLog, viewModel: AiRequestLogViewModel) {
    val context = LocalContext.current
    val sections: List<Pair<String, String>> = remember(log.id) { buildSections(log) }
    val fullText: String = remember(log.id) {
...
private fun buildSections(log: AiRequestLog): List<Pair<String, String>> {
    ...
    val meta = buildString {
        ...
        append("duration: ").append(String.format(Locale.US, "%.1f s", log.durationMs / 1000f)).append('\n')
        append("tokens: in=").append(log.promptTokens)
            .append(" out=").append(log.outputTokens)
            .append(" thought=").append(log.thoughtTokens)
        if (log.truncated) append("\n(truncated)")
    }
    sections += "META" to meta

    log.params?.let {
        sections += "PARAMS" to buildString {
            append("temperature: ").append(it.temperature).append('\n')
            append("topP: ").append(it.topP).append('\n')
            append("topK: ").append(it.topK).append('\n')
            append("maxTokens: ").append(it.maxTokens).append('\n')
            append("presencePenalty: ").append(it.presencePenalty).append('\n')
            append("frequencyPenalty: ").append(it.frequencyPenalty)
        }
    }
